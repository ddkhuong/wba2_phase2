...
public class NachrichtenService {
		...
		public Nachrichten unmarshalFeed() throws JAXBException, FileNotFoundException{
			JAXBContext context = JAXBContext.newInstance(Nachrichten.class);
			   Unmarshaller um = context.createUnmarshaller();
			   Nachrichten news = (Nachrichten) um.unmarshal(new FileInputStream("src/FeedXML.xml"));
			   return news;
		}
	   ...
	   public Nachrichten getNachrichten() throws JAXBException, FileNotFoundException
	   {
		...
		@Path("/{news_id}")
		@Produces(MediaType.APPLICATION_XML)
		public News getNews(@PathParam("news_id") String news_id) throws JAXBException, FileNotFoundException{
	   ...
	   @Path("{news_id}")
	   @Produces(MediaType.APPLICATION_XML)
	   public Response deleteNews(@PathParam("news_id") String news_id) throws JAXBException, FileNotFoundException{
	   ...
	   @Produces(MediaType.APPLICATION_XML)
	   public Response postNachricht(News n) throws JAXBException, FileNotFoundException {
	   ...
	   @Produces(MediaType.APPLICATION_XML)
	   public Kommentare getKommentare(@PathParam("news_id") String news_id) throws JAXBException, FileNotFoundException{
	   ...
	   public Kommentar getKommentar(@PathParam("news_id") String news_id,
			   						@PathParam("kommentar_id") String kommentar_id) throws JAXBException, FileNotFoundException{
	   ...
	   public Response postKommentar(@PathParam("news_id") String news_id,Kommentar k) throws FileNotFoundException, JAXBException{
		   ...
		   int lastid = Integer.parseInt(k_list.getKommentar().get(size-1).getId());
		   ...
		   news_daten.getFeed().getNews().get(Integer.parseInt(news_id)-1).getKommentare().getKommentar().add(k);
		   marshalFeed(news_daten);
	   ...
	   public Response deleteKommentar(@PathParam("news_id") String news_id,
	   		...
	   		for(int i=0; i<k_list.getKommentar().size(); i++){
	   			if(kommentar_id.equals(k_list.getKommentar().get(i).getId())){
	   				news_daten.getFeed().getNews().get(Integer.parseInt(news_id)-1).getKommentare().getKommentar().remove(i);
	   				marshalFeed(news_daten);
