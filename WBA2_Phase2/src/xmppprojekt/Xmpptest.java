...
public class Xmpptest {
	...
	public void start() throws XMPPException {
		...
		System.out.println("Hallo "
				+ connection.getUser().substring(0,
						connection.getUser().indexOf("@"))
				+ " Sie haben sich erfolgreich verbunden");
	...
	private void hauptmenue(String serienName) throws XMPPException {
		/*
		 * Scanner in = new Scanner(System.in); int teste=0;
		 * 
		 * int wahl=0;
		 * 
		 ...
		 * System.out.println("6 Publish"); System.out.println("7 Ende"); wahl =
		 * in.nextInt();
		 * 
		 * switch(wahl){ case 1: System.out.println("Serie Erstellen");
		 * createSerie(serienName); hauptmenue(serienName); break; case 2:
		 * System.out.println("Serien Infos"); getLeafInfo(serienName);
		 * getSerien(); hauptmenue(serienName); break; case 3:
		 * System.out.println("Serie Abonnieren"); serieSubscribe(serienName);
		 * hauptmenue(serienName); break; case 4:
		 * System.out.println("Abo Kündigen"); serieUnsubscribed(serienName);
		 * hauptmenue(serienName); break; case 5: deleteSerie(serienName);
		 * hauptmenue(serienName); break; case 6: publishSerie(serienName);
		 * hauptmenue(serienName); break; case 7:
		 * System.out.println("Bis Bald"); break; default:
		 * System.out.println("Falsche Eingabe"); hauptmenue(serienName); break;
		 * 
		 * }
		 */
	}

	public void connect(JTextField usernameTextfield,
	...
	public void register(JTextField usernameTextfield,
			JPasswordField passwordTextfield) {
		this.usernameTextfield = usernameTextfield;
		this.passwordTextfield = passwordTextfield;
		try {
	...
	public void aboDeleteButton(Abos abo, JList addAboList,
			JList subscriptionsList) {
	...
	public void publishSerie(JList serienList, JTextArea seriePayloadTextarea) {
		...
		if (serienListe.isSelectionEmpty() || "".equals(payloadText)) {
			...
			try {
				...
				PayloadItem<SimplePayload> item = new PayloadItem<>(null, p); // man
																				// könnte
																				// auch
																				// =
																				// new
																				// PayloadItem<SimplePayload>(p);
				node.publish(item);
				// node.send(item);
	...
	public void getLeafInfo(String serienName) throws XMPPException {
		...
		DiscoverItems discoItems = discoManager
				.discoverItems("pubsub.localhost");
		java.util.Iterator<org.jivesoftware.smackx.packet.DiscoverItems.Item> it = discoItems
				.getItems();
		while (it.hasNext()) {
	...
	public void serieUnsubscribed(String serienName) {
		try {
			...
			Logger.getLogger(Xmpptest.class.getName()).log(Level.SEVERE, null,
					ex);
		}
	}
		...
		for (int i = 0; i < serien.size(); i++) {
			...
			if (size > 0) {
				for (int j = 0; j < size; j++) {
					System.out.println("Abonniert von : "
							+ node.getSubscriptions().get(j).getJid());
				}
			}
		}
	}
	...
	public ArrayList<LeafNode> getSerienListe() {
		serien.clear();
		ServiceDiscoveryManager disco_manager = ServiceDiscoveryManager
				.getInstanceFor(connection);
